...
export function RedeemDialog({ children }: RedeemDialogProps) {
  ...
  const handleRedeem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "학습권 충전 완료",
        description: data.message || `${data.creditsAdded} 학습권이 충전되었습니다.`,
      });
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] rounded-2xl p-6 shadow-[0_20px_60px_rgba(0,0,0,0.12)] border-none">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-[#191F28]">학습권 등록</DialogTitle>
          <DialogDescription className="text-[#8B95A1] mt-2">
            관리자로부터 발급받은 코드를 입력하여<br/>학습권을 충전하세요.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleRedeem}>
          <div className="grid gap-5 py-6">
            <div className="space-y-2">
              <Label htmlFor="code" className="text-[#4E5968] font-semibold">학습권 코드</Label>
              <Input
                id="code"
