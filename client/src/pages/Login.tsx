...
function isFlutterWebView(): boolean {
  ...
  const isAndroidWebView = /Android.*wv/.test(userAgent) || 
                           (/Android/.test(userAgent) && /Version\/[\d.]+/.test(userAgent) && !/Chrome\/[\d.]+/.test(userAgent));
  const isIOSWebView = /iPhone|iPad|iPod/.test(userAgent) && !/Safari/.test(userAgent);
  const hasFlutterIndicator = userAgent.includes('Flutter') || 
                              userAgent.includes('Dart') ||
                              userAgent.includes('KonnectApp');
  return isAndroidWebView || isIOSWebView || hasFlutterIndicator;
}
...
export default function Login() {
  ...
  const isInApp = useMemo(() => isFlutterWebView(), []);
